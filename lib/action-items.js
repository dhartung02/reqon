...
function computeActionItems(rows, ctx) {
  ...
  for (const r of live) {
    ...
    // suppressed when lastcontact >= interview (contact after the interview = note sent)
    const ivd = daysAgo(r.interview, today);
    const noteSent = r.lastcontact && r.interview && r.lastcontact >= r.interview;
    if (INTERVIEW.has(status) && ivd != null && ivd >= 0 && ivd <= 2 && !noteSent) {
