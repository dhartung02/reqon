// ---------------------------------------------------------------------------
// Unified action-item model (ROADMAP · P2.1)
//
...
function computeActionItems(rows, ctx) {
  ...
  for (const r of live) {
    ...
    // suppressed once the candidate checks the "Thank-you sent" checkbox (sets thankYouSent date)
    const ivd = daysAgo(r.interview, today);
    if (INTERVIEW.has(status) && ivd != null && ivd >= 0 && ivd <= 2 && !r.thankYouSent) {
    ...
    // apply next — verified, top-tier, still-open, not-yet-applied. Mirrors the board's isApplyNext()
    // (and the app's actionItems.ts) so the Today "Apply next" card and this panel section show the
    // SAME roles. Strong-but-unverified roles deliberately surface under "verify_role" instead of
    // being double-counted here — verify first, then they graduate into apply-next.
    if (status === 'Not Applied' && ['A', 'B'].includes(r.tier) && r.conf === 'verified'
        && !['closed', 'lead', 'unknown'].includes(r.reqCheck || '')) {
      const e = ev(r);
      push(Object.assign({ id: 'apply-' + id, type: 'apply_next', severity: e >= 7 ? 'high' : 'medium', priority: 60 + Math.round(e), source: 'score',
        reason: (r.tier === 'A' ? 'Strong' : r.tier === 'B' ? 'Possible' : r.tier === 'C' ? 'Long shot' : 'Unknown') + ' · EV ' + e + ' · verified — apply next.', cta: { label: 'Open role', target: 'role-detail' } }, base));
    }
