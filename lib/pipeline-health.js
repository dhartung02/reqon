// ---------------------------------------------------------------------------
// Pipeline health score (ROADMAP-V3 · P2.6)
//
...
function computePipelineHealth(rows, ctx) {
  ...
  if (applyReady.length < 3) { const w = applyReady.length === 0 ? 30 : 18; score -= w; risks.push({ weight: w, text: applyReady.length === 0 ? 'No apply-ready Tier A/B roles in the queue.' : `Only ${applyReady.length} apply-ready Tier A/B role${applyReady.length === 1 ? '' : 's'} queued.` }); }
  ...
  if (applyReady.length < 3) recommendations.push({ text: 'Run the scout to find fresh Tier A/B roles.', action: 'run_scout' });
