// ---------------------------------------------------------------------------
// Tenant-scoped store (ROADMAP · PR 0 · slice 1)
//
...
// Where persisted data lives. Defaults to the repo (legacy/local). On an ephemeral host (Render),
// set REQON_DATA_DIR to a mounted disk so data.json, backups, profiles, settings, jobs, per-user
// namespaces, etc. survive deploys. Code stays under ROOT; only data roots here.
const DATA_ROOT = process.env.REQON_DATA_DIR ? path.resolve(process.env.REQON_DATA_DIR) : ROOT;
const AGENT = path.join(DATA_ROOT, 'agent');
...
function pathsFor(userId) {
  if (isLegacy(userId)) {
    return {
      userId: OWNER,
      dir: ROOT,
      data: process.env.DATA_FILE ? path.resolve(process.env.DATA_FILE) : path.join(DATA_ROOT, 'data.json'),
      backups: process.env.BACKUP_DIR ? path.resolve(process.env.BACKUP_DIR) : path.join(DATA_ROOT, 'backups'),
  ...
  const base = path.join(DATA_ROOT, 'data', 'users', slug(userId));
  return {
    userId: slug(userId),
    dir: base,
