// ---------------------------------------------------------------------------
// User registry + auth (ROADMAP · PR 0 · slice 2)
//
...
// Persist the registry (accounts, password hashes, session-signing secret) on the mounted disk when
// REQON_DATA_DIR is set — otherwise it lands in the ephemeral code dir and every Render redeploy
// wipes all accounts + invalidates every session. Falls back to the repo root for local/dev, so
// existing single-box behavior is unchanged. Mirrors lib/store.js DATA_ROOT.
const DATA_ROOT = process.env.REQON_DATA_DIR ? path.resolve(process.env.REQON_DATA_DIR) : ROOT;
const LEGACY_USERS_FILE = path.join(ROOT, 'users.json');
const USERS_FILE = path.join(DATA_ROOT, 'users.json');
const slug = u => String(u || '').toLowerCase().trim().replace(/[^a-z0-9_-]/g, '').slice(0, 40);

// One-time migration: if we're now disk-backed but the disk has no registry yet, copy a pre-existing
// legacy root users.json onto the disk so accounts survive the move. Never overwrites an existing
// disk file (idempotent + safe to call on every boot). Best-effort: load() recreates a fresh
// registry if anything here fails.
function migrateLegacyUsersFile() {
  try {
    if (DATA_ROOT === ROOT) return;                 // same location — nothing to migrate
    if (fs.existsSync(USERS_FILE)) return;          // disk already has one — never clobber it
    if (!fs.existsSync(LEGACY_USERS_FILE)) return;  // no legacy file to bring over
    fs.mkdirSync(path.dirname(USERS_FILE), { recursive: true });
    fs.copyFileSync(LEGACY_USERS_FILE, USERS_FILE);
    fs.chmodSync(USERS_FILE, 0o600);
  } catch (e) { /* best-effort; never leak file contents to logs */ }
}
...
function save(db) {
  fs.mkdirSync(path.dirname(USERS_FILE), { recursive: true });   // disk mount may start empty
  const tmp = USERS_FILE + '.tmp';
  fs.writeFileSync(tmp, JSON.stringify(db, null, 2), { mode: 0o600 });
  fs.renameSync(tmp, USERS_FILE);
}
// SESSION_SECRET env var overrides the auto-generated per-file secret so that
// separate processes (reqon-api + reqon-cloud on Render) can verify each other's
// signed cookies without sharing a disk. When not set, falls back to the value
// persisted in users.json — works fine for single-process deployments.
function secret() { return process.env.SESSION_SECRET || load().secret; }
...
// --- early-access invite tokens (signed, expiring) ---
// A stateless invite for the create-account page: base64url({email, exp}) + '.' + HMAC(payload).
// Single-use in practice — accepting an invite creates the account, after which the email is taken
// and the token can't mint a second one. Signed with the same secret as sessions.
function _signInvite(payload) { return crypto.createHmac('sha256', secret()).update(payload).digest('base64url'); }
function makeInvite(email, ttlDays = 14) {
  const exp = Date.now() + Math.max(1, ttlDays) * 86400000;
  const payload = Buffer.from(JSON.stringify({ e: String(email || '').toLowerCase().trim(), x: exp })).toString('base64url');
  return payload + '.' + _signInvite(payload);
}
function verifyInvite(token) {
  if (!token || typeof token !== 'string') return null;
  const i = token.lastIndexOf('.');
  if (i < 1) return null;
  const payload = token.slice(0, i), sig = token.slice(i + 1), expSig = _signInvite(payload);
  try { if (!(sig.length === expSig.length && crypto.timingSafeEqual(Buffer.from(sig), Buffer.from(expSig)))) return null; } catch (e) { return null; }
  let obj; try { obj = JSON.parse(Buffer.from(payload, 'base64url').toString('utf8')); } catch (e) { return null; }
  if (!obj || !obj.e || !obj.x || Date.now() > obj.x) return null;
  return { email: obj.e, exp: obj.x };
}

// --- registry ops ---
const publicView = u => u && ({ id: u.id, username: u.username, displayName: u.displayName, email: u.email || '', role: u.role, createdAt: u.createdAt, disabled: !!u.disabled, useSharedKey: !!u.useSharedKey, license: u.license || '' });
...
function create({ username, displayName, password, email = '', role = 'user', useSharedKey = false, license = '' }) {
  const db = load();
  ...
  // license is the user's freemium package grant ("cloud", "ai", "cloud,ai", "pro", "owner"); admin-only.
  const u = { id, username: String(username).trim(), displayName: String(displayName || username).trim(),
    ...
    createdAt: new Date().toISOString(), disabled: false, useSharedKey: !!useSharedKey, license: String(license || '').trim() };
...
function update(id, patch) {
  ...
  if (typeof patch.license === 'string') u.license = patch.license.trim();
...
module.exports = {
  ...
  setToken, userByToken, hasToken, makeInvite, verifyInvite,
};
